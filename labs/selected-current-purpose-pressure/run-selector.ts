...
async function main() {
  ...
  const campaignId = `selector-${new Date().toISOString().replaceAll(/[:.]/g, "-")}`
  ...
  for (let trial = 1; trial <= TRIALS; trial += 1) {
    const order = trial === 1 ? selectorScenarios : [...selectorScenarios].reverse()
    for (const scenario of order) {
      if (spentUsd + RESERVED_PER_CALL_USD > budgetUsd) {
        throw new Error(`ALS-022B cannot reserve another call within $${budgetUsd.toFixed(2)}`)
      }
      ...
      try {
        result = await generateText({
          ...
          prompt: renderSelectorScenario(scenario),
          output: Output.json({
            name: "selected_current_learning_purpose",
            description:
              "One untrusted control-only proposal to adopt one visible Agenda purpose or none.",
          }),
      ...
      const assessment = result
        ? assessSelectorOutput(scenario, result.output)
        : {
            passed: false,
      ...
      const record = {
        ledgerId: LEDGER_ID,
        protocolRevision: SELECTOR_PROTOCOL_REVISION,
        sampleId,
        trial,
        scenario,
        model: {
      ...
      if (failure) {
        ...
        throw new Error(`ALS-022B stopped after ${sampleId}: ${failure.message}`)
      }
    }
  }
  ...
  process.stdout.write(
    `Completed ${results.length}; exact ${passed}/${results.length}; estimated cost $${spentUsd.toFixed(6)}; artifacts ${campaignRoot}\n`,
  )
}
...
function readBudget() {
  ...
  const required = selectorScenarios.length * TRIALS * RESERVED_PER_CALL_USD
  if (
    !raw ||
    !Number.isFinite(value) ||
    value < required ||
    value > MAX_CAMPAIGN_USD
  ) {
    throw new Error(
...
async function writeSummary(
  ...
  const assessments = results.map(
    (record) => record.assessment as {
      ...
      semanticConstraintCorrect: boolean
    },
  )
  const byScenario = Object.fromEntries(
    selectorScenarios.map((scenario) => [
      scenario.id,
      results
        .filter(
          (record) =>
  ...
  await atomicJsonWrite(join(campaignRoot, "summary.json"), {
    ...
    decisionCorrect: assessments.filter((item) => item.decisionCorrect).length,
    ...
    semanticConstraintCorrect: assessments.filter(
      (item) => item.semanticConstraintCorrect,
    ).length,
    byScenario,
    budgetUsd,
