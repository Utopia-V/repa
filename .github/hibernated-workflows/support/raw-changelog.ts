...
const team = [
  ...(await Bun.file(new URL("../../TEAM_MEMBERS", import.meta.url))
    .text()
    .then((x) => x.split(/\r?\n/).map((x) => x.trim()))
...
if (import.meta.main) {
  ...
  if (values.help) {
    console.log(`
Usage: bun .github/hibernated-workflows/support/raw-changelog.ts [options]
...
Examples:
  REPA_RUN_HIBERNATED_OPENCODE_AUTOMATION=1 bun .github/hibernated-workflows/support/raw-changelog.ts
  REPA_RUN_HIBERNATED_OPENCODE_AUTOMATION=1 bun .github/hibernated-workflows/support/raw-changelog.ts --from 1.0.200
  REPA_RUN_HIBERNATED_OPENCODE_AUTOMATION=1 bun .github/hibernated-workflows/support/raw-changelog.ts -f 1.0.200 -t 1.0.205
`)
    process.exit(0)
  }
