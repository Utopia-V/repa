...
// Inactive fork evidence for the hibernated OpenCode publish workflow.

import { Script } from "@opencode-ai/script"
import { $ } from "bun"

if (process.env.REPA_RUN_HIBERNATED_OPENCODE_AUTOMATION !== "1")
  throw new Error("Refusing to run hibernated upstream release automation")

const output = [`version=${Script.version}`]
const sha = process.env.GITHUB_SHA ?? (await $`git rev-parse HEAD`.text()).trim()

if (!Script.preview) {
  await $`bun .github/hibernated-workflows/support/changelog.ts --to ${sha}`.cwd(process.cwd())
