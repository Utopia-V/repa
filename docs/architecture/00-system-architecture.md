...
The control view is event-triggered and receding-horizon: at an admitted
interaction it chooses or proposes a useful current move and near-term
constraints or attention, then chooses again only when Repa later has another
real observation or accepted correction. It does not assume continuous use,
exclusive observation, or execution of an earlier proposal, and it does not
compile a whole course into a rigid workflow. This is the same reason plan,
...
transaction facility. Internal modules are separated by the meaning they own,
not deployed as services. One application command may compose several local
owners atomically only when it genuinely owns all consequences; mere sequence
or convenience does not merge independent lifecycles. This avoids network
protocols, distributed locks, and eventual consistency inside one learner's
local system without making every adjacent write all-or-none.
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------- |
| routine current view | relevant Goals/Course candidates, route anchor/current focus, due future attention, exact Assignment constraints, current working-plan constraints/staleness, time budget, active steering, compact source references | compiled automatically when relevant |
| current-move detail  | route neighborhood, exact material range, current Assignment/plan inputs, recent activity or evidence that changes the move                     | selected during composition or read through a tool |
...
plan completion, Assignment lifecycle, or a durable active activity. After interruption, provider
...
A domain may define an explicit correction occurrence whose stable runtime
binding is the exact issuing model operation. That operation identity is a
command-specific causal address only when the domain's typed correction arm,
exact entity-head anchor, source/provenance rules, and repeat semantics admit it;
generic `agent_action` provenance or ordinary resampling does not automatically
make every later model operation a new semantic occurrence or an unbounded
create slot.
...
requires a new command-valid admitted occurrence under the owning domain's
address contract. Finite model/tool budgets remain code-enforced.
...
overdue, expired, and plan-age relations from stored times and the trusted
clock. This calculation creates no activity observation, progress update,
Assignment lifecycle transition, or recomputed plan by itself.
