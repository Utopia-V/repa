...
Recalibrated: 2026-07-14, 2026-07-17, 2026-07-21, and 2026-07-29
...
or failure. This responsibility does not preselect an implementation category
or program/model ratio.
...
- **Gate 21A — Tutor move selection and flow continuity:** the representative
  ...
  interruption, or restart. The Gate must compare relevant candidates,
  including ordinary Tutor-LLM, bounded control-only model sample,
  program-policy, and mixed mechanisms, without pre-authorizing a universal
  selector, classifier, scheduler, durable active-purpose record, or fixed
  teaching workflow.
...
| 14. Learner navigation continuity                                                                                            | Optional learner-controlled default Course preference plus one independent route anchor per Course, both versioned and distinct from current focus, completion, evidence, and mastery.                                                                     | Depends on Course/View identity; model-issued writes reuse Gate 8 and the durable Turn source.                                                                                             | One active Course, a global current item, implicit directory/model preference mutation, a Gate-specific extra prompt under effective `allow`, bypassing configured `ask`, permission masquerading as semantic acceptance, current-state reconstruction of historical acknowledgement, fabricated legacy confirmation/locator facts, or mastery inference. | Independent Courses; a closed versioned candidate union distinguishes explicitly classified legacy V1 rows, direct learner request, and exact source-bound proposal acceptance while capability allow/ask/deny settles separately; an accepted proposal cites one host-prepared non-mutating completed Tool Part rather than plain prose or a provider-executed call; a physically new invocation first resolves any committed semantic duplicate/conflict, which settles without current-owner validation, candidate authorization, or capability evaluation; only a genuinely new V2 candidate atomically reserves authorization with physical admission before capability evaluation; effective-`ask` evaluation plus issue is atomic, request ID exists iff issue is durable, and final candidate settlement revalidates/seals rather than first creating authorization; candidate recovery maps no-evaluation/issued-no-reply/durable-reply without re-prompting or applying an uncommitted effect; direct authorization receives no second Gate-forced prompt under `allow`; immutable V2 set/change/clear acknowledgement carries symmetric stable `from`/`to` locators, while unavailable V1 version/selection/confirmation facts stay explicit, and neither replay consults current Course state; a frozen-current-V12 forward migration preserves raw V1 replay/pending recovery, proves fresh/upgraded V13 parity, and may replace only the physically shared no-effect trigger while preserving route-owned DDL, its route branch, rows, and behavior; request-specific Course use without preference mutation; stale anchors, deleted source transcript, restart, and no false semantic promotion. |
...
| 18. Learning context and Session continuation                                                                                | Every admitted released-v1 interactive Tutor/Agent model operation receives a bounded, immutable, revision-exact projection with lazy detail; fresh/resumed Sessions reuse relevant learning state without importing another Session's transcript. Program-owned internal model operations retain their narrow purpose contracts. | Consumes real material, navigation, steering, Goal, Turn, and policy records; it performs no domain writes and reuses rather than reinterprets any exact steering cut already bound by Gate 15. | A second context database, eager full-state/transcript import, full interactive learning context for internal title/compaction/representation operations, durable summary as truth, choosing the Tutor move, a newly invented soft-workspace-memory subsystem, or a new runtime. | Bounded cross-Course context-scope resolution and candidate projection without choosing the Tutor move; exact policy/authority revisions, preference non-mutation, missing/truncated detail, non-mutating continuation, restart, compaction, fresh Session behavior, and carrier coverage without widening internal-operation authority. |
...
| 21A. Tutor move selection and flow continuity                                                                                | Given one bounded current learning situation and exact learner request, Tutor composition normally selects or proposes a useful current move, records enough exact basis and candidate coverage for later inspection, and selects again after correction, interruption, provider failure, restart, or relevant state change. Clarification is not the default escape; it is warranted when materially different choices cannot be handled safely as a transparent reversible default. This Gate adds no domain schema by default. | Follows Gate 18 and every still-admitted baseline pressure producer through Gate 21; context visibility alone is not control, and Gate 22 needs an actual selection result to inspect. The suffix preserves the published Gate 22/23 meanings. | A universal scheduler, mandatory classifier or extra model call, fixed teaching workflow, pedagogy taxonomy, durable active-purpose/activity record, automatic progress, silent durable priority, or eliminating all clarification. | Before contract, compare relevant candidates—including ordinary Tutor-LLM, bounded control-only model sample, bounded program-policy, mixed mechanisms, and any other evidenced alternative—on the same representative collision set. Evidence covers an exact request, an obvious reversible default, one genuinely unsafe ambiguity, multiple Courses/Goals, due future attention, planning, steering, recent Interaction, any Gate-19-admitted learner-record/adaptation pressure, complete versus truncated candidates, current-request override, correction, latency/cost, cancellation, provider failure, restart, and no internal-ID or state-management Turn. A model-qualified result must change the actual Tutor move; deterministic checks preserve legality, provenance, omission truth, no fabricated durable effects, and re-entry without blind replay. |
