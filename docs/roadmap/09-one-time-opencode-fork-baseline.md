...
Evidence: [OpenCode one-time fork audit](../research/opencode-one-time-fork-audit-2026-07-13.md)
...
| 3. Repa identity isolation | Change only binary/product identity and application-owned paths so a fresh Repa launch cannot read or write OpenCode state. [Passed 2026-07-13 at fork commit `0ffed9f62`.](../research/opencode-fork-gate-03-repa-identity-isolation-2026-07-13.md) | Exercise existing OpenCode homes, missing/unwritable paths, interrupted first launch, and database-name collision. Revert the identity checkpoint on failure. |
...
commit `0ffed9f62`. Gate 4 is the next authorized engineering move. Later gate
contracts are refined from the evidence immediately before them rather than
guessed now from file names.
