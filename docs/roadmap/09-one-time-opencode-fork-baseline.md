...
Agent; `/learn` may be an optional shortcut but is not an admission requirement.
...
Recalibrated: 2026-07-14, 2026-07-17, 2026-07-21, 2026-07-29, and 2026-07-30
...
| 17. [Natural-language learning bootstrap](../research/opencode-fork-gate-17-natural-language-learning-bootstrap-2026-07-22.md) | The ordinary released-v1 Agent can turn a deliberate natural-language request into only the Course/View state currently needed, can separately admit exact source/material state when the learner adopts it, and can teach immediately. `/learn` may be an optional discoverability shortcut but is not required and supplies no domain authority. Separately authorized Goal, steering, or other effects remain governed by their owners. A model-authored route is visibly provisional by exact provenance, not by a Course lifecycle state. | Needs model-visible Course reads, Course authority, command settlement, durable Turn, and a request-bound creation/effect identity; the material path also uses Artifact and, when needed, Representation and Map/alignment authorities. | A second natural-language interpreter, universal CRUD/command bus, shelling into deterministic CLI mutation, hidden import from read/search/web research, mandatory prefix/Goal/LearningSpace, eager whole-route construction, automatic ordinal/progress advancement, or an implicit Session/queue/macro-activity lifecycle. | Fresh LearnerHome traces with and without local material; ordinary Agent distinction among create, continue, revise, teach-only, and meaningful ambiguity; exact replay/conflict for creation; one bounded atomic local composition versus truthful staged settlement where later work depends on earlier results; visible partial settlement; transient web research remaining unadmitted; immediate explanation/demonstration; correction preserving history/provenance; and generic reads remaining zero-write. |
