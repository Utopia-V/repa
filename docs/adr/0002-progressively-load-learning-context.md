...
完整语境补回后，最终模型输入的容量处理与压缩协同仍需明确并验证，包括不同模型窗口、连续更新及压缩后的实际输入。准备失败保留请求、不能静默替换背景的既有原则继续适用，具体超限处理与默认展开策略尚未确定。后续优先复用 Pi 的压缩流程，并参考 [Codex 的工作历史缩减与必要上下文重建](https://github.com/openai/codex/blob/3d2ee51ca2d5db578f328aa75e20aa22c0197c9a/codex-rs/core/src/compact.rs)，据实际接入结果细化。
