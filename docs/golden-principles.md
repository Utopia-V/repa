...
1. **文档与代码同源**【机械化】 — 行为或结构变化必须同步文档；校验规则以文档为唯一规则来源（如 [`ARCHITECTURE.md`](../ARCHITECTURE.md) 边表）。强制：`check-architecture` 解析边表、`check-docs` 验证链接。
2. **分层方向不可逆行**【机械化】 — `src/` 只允许边表登记的模块边；新依赖先入册再使用。强制：`check-architecture`。
3. **仓库是唯一事实来源**【机械化】 — 决定进 [`docs/adr/`](adr/)，跨任务工作进 [`docs/exec-plans/`](exec-plans/README.md)，待办走 GitHub Issues；`src/` 内不写 TODO/FIXME。强制：`check-architecture` 的 TODO 扫描。
4. **文件保持小而单一职责**【机械化】 — `src/` 单文件 ≤ 600 行；超过即拆层或抽共享模块。文件是 agent 的认知单元，大小即可读性。
...
3. 扫描文档中的 `_Avoid_` 近义词与失联链接（链接脚本已覆盖）。
