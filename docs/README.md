...
Status: Gates 0–10 are closed. Gate 11's
[contract and implementation record](research/opencode-fork-gate-11-readable-representation-lineage-2026-07-17.md)
is independently accepted after closing `G11-CT-001` through `G11-CT-012` and
`G11-IE-001` through `G11-IE-003`. Deterministic/local evidence, both packaged
Windows families, and the authorized real configured-multimodal-provider run
pass. The reviewed working tree is ready for a separately authorized
integration commit that fixes implementation provenance; until then Gate 11 is
not formally closed, and Gate 12 has not started. The concurrent Roadmap 09 edit
was included in the accepted closure snapshot and did not change Gate 11's
number or boundary.
...
| 11. [Readable representation lineage](research/opencode-fork-gate-11-readable-representation-lineage-2026-07-17.md)       | Gives one exact Artifact Revision an optional immutable readable derivation with truthful conversion, drift, availability, exact bounded reads, and cleanup semantics. | Contract/theory and implementation/evidence are independently accepted; all twelve contract findings and all three implementation findings are closed. The accepted evidence includes the inherited OpenAI OAuth configured-model path and both packaged Windows families. The reviewed working tree is ready for separately authorized integration, but Gate 11 remains open until that commit fixes implementation provenance. The boundary includes one local PDF text-layer producer plus one optional user-configured multimodal-model producer and no baseline Repa-owned OCR. Current Roadmap 09 preserves its number and boundary. |
...
purposes; Gate 11's accepted correction is now present in the independently
reviewed working tree as the fourth `representation` purpose, pending only Gate
11's separately authorized integration provenance. Gate 4 rejects
...
7–10 remain closed; both Gate 11 review layers are accepted and its integration
commit remains pending. The 2026-07-17 global audit preserved Gate 11 and replaced
the unstarted route after it with Gates 12–23. The revised route adds durable
Turn truth, natural-language bootstrap, retained steering, Learner Goal,
learning-native terminal inspection/correction, and final integrated product
acceptance while keeping selective deep deletion in an explicit post-baseline
Data Lifecycle capability.
...
- [Independently accepted Gate 11 readable representation lineage record](research/opencode-fork-gate-11-readable-representation-lineage-2026-07-17.md)
