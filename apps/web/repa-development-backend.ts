...
function connectionFile(): string {
  return path.join(
    os.tmpdir(),
    `repa-web-${process.getuid?.() ?? os.userInfo().username}`,
    `${process.pid}.json`,
  );
}
...
async function readEndpoint(file: string): Promise<Endpoint | undefined> {
  try {
    ...
    if (
      ...
      throw new Error("Web 开发后端连接文件无效。");
    }
...
export async function startRepaDevelopmentBackend(): Promise<ClientConnection> {
  ...
  const existing = await readEndpoint(file);
  ...
  await new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      child.kill();
      reject(new Error(`Web 开发后端启动超时，日志：${logFile}`));
    }, 15_000);
    ...
    child.once("exit", (code) => {
      clearTimeout(timer);
      reject(new Error(`Web 开发后端启动失败（${code}），日志：${logFile}`));
    });
  });
  child.disconnect();
  child.unref();
  ...
  if (!endpoint || !isAlive(endpoint.pid)) {
    throw new Error(`Web 开发后端没有发布可用连接，日志：${logFile}`);
  }
