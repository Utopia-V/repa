...
// etc.).
//
// The visual language reuses the inherited neutral design tokens while the
// callback copy and wordmark identify the independent Repa product.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Repa is now connected to ${escapeHtml(provider)}.` : "Repa is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Repa couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Repa couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Repa.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Repa</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Repa couldn't finish connecting to "+PROVIDER+"."):"Repa couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Repa."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Repa is now connected to "+PROVIDER+"."):"Repa is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .brand .wordmark { color: var(--oc-text-strong); font-size: 1.15rem; font-weight: 700; letter-spacing: 0.16em; }
...
const WORDMARK = `<span class="wordmark" aria-label="Repa">REPA</span>`
