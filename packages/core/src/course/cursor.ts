export * as CourseCursor from "./cursor"
...
export const Endpoint = Schema.Union([
  Schema.Literal("courses"),
  Schema.Literal("views"),
  Schema.Literal("revisions"),
  Schema.Literal("items"),
  Schema.Literal("mapping_groups"),
  Schema.Literal("mapping_sources"),
  Schema.Literal("mapping_targets"),
  Schema.Literal("reuse_citations"),
])
...
const Payload = Schema.Struct({
  ...
  includeWithdrawn: Schema.Boolean,
...
export function options(input: PageOptions | undefined, scope: Scope) {
  return Effect.gen(function* () {
    ...
    if (Option.isNone(payload)) {
      return yield* new InvalidCursorError({ detail: "Cursor is malformed" })
    }
    if (
      payload.value.endpoint !== scope.endpoint ||
      payload.value.parent !== scope.parent ||
      payload.value.includeWithdrawn !== scope.includeWithdrawn
    ) {
      return yield* new InvalidCursorError({ detail: "Cursor belongs to a different read scope" })
    }
...
export function next(scope: Scope, key: readonly (string | number)[]) {
  return base64Encode(
    JSON.stringify({
      ...
      parent: scope.parent,
      includeWithdrawn: scope.includeWithdrawn,
      key,
    } satisfies Payload),
  )
}
