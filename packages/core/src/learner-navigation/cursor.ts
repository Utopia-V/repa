export * as LearnerNavigationCursor from "./cursor"

import { Effect, Option, Schema } from "effect"
import { base64Decode, base64Encode } from "../util/encode"
import { InvalidCursorError, type PageOptions } from "./schema"

const Endpoint = Schema.Literals(["default_history", "anchor_history", "anchored_courses"])
...
const Payload = Schema.Struct({
  ...
  throughSequence: Schema.Int.check(Schema.isGreaterThanOrEqualTo(0)),
  key: Schema.Array(Schema.Union([Schema.String, Schema.Number])),
})
type Payload = typeof Payload.Type

export type Scope = Readonly<{ endpoint: Endpoint; parent: string }>
...
export function options(input: PageOptions | undefined, scope: Scope) {
  return Effect.gen(function* () {
    ...
    if (input?.cursor === undefined) return { limit, throughSequence: undefined, key: undefined }
    ...
    if (payload.value.endpoint !== scope.endpoint || payload.value.parent !== scope.parent) {
      return yield* new InvalidCursorError({ detail: "Cursor belongs to a different navigation read scope" })
    }
    return {
      limit,
      throughSequence: payload.value.throughSequence,
      key: [...payload.value.key],
    }
  })
}

export function next(scope: Scope, throughSequence: number, key: readonly (string | number)[]) {
  return base64Encode(
    JSON.stringify({
      version: 1,
