...
import { install as installSchemaExtrasV22 } from "@opencode-ai/core/database/schema-extras-v22"
...
function materialMapLayer(database: Database.Interface) {
...
const advisoryPlanSuggestionTables = [
...
function initializeDatabaseV19(db: TestDatabase) {
  return Effect.gen(function* () {
    yield* initializeDatabaseV11(db)
    yield* DatabaseMigration.apply(db, {
      path: "frozen-gate19.db",
      migrations: databaseV19Migrations,
    })
  })
}

function initializeDatabaseV20(db: TestDatabase) {
  return Effect.gen(function* () {
    yield* initializeDatabaseV11(db)
    yield* DatabaseMigration.apply(db, {
...
function initializeDatabaseV22(db: TestDatabase) {
  return Effect.gen(function* () {
    yield* initializeDatabaseV11(db)
    yield* DatabaseMigration.apply(db, {
...
function prepareLegacyLearningContextCut(input: {
  ...
  return LearningContext.prepareCut({} as Transaction, {
    ...
    Effect.map((prepared) => {
      ...
      const futureLazyReadID = generation === 1 ? "learner_response_evidence_read" : "future_attention_read"
      const providerToolSurface = input.collideWithFutureLazyReadID
        ? LearningContext.bindProviderToolSurface({
            ...
            definitions: [
              {
                id: futureLazyReadID,
                value: { type: "function", name: futureLazyReadID, description: "legacy custom tool" },
              },
            ],
          })
        : undefined
      ...
      throw new Error(`Frozen learning-context generation ${generation} byte accounting did not converge`)
    }),
  )
}
...
function seedFrozenLearningContextCut(db: TestDatabase, generation: 2 | 3 | 4 | 5 = 2) {
  return Effect.gen(function* () {
    ...
    yield* db.run(sql`
      ...
      VALUES (${sessionID}, ${ProjectV2.ID.global}, ${sessionID}, '/', ${`Frozen learning context v${generation}`}, 'test', 100, 100)
    `)
    yield* db.run(sql`
    ...
    yield* db.run(sql`
      ...
      VALUES (${occurrencePartID}, ${occurrenceMessageID}, ${sessionID}, 100, 100, ${JSON.stringify({ type: "text", text: `frozen learning context v${generation}` })})
    `)
    ...
    yield* db.transaction((tx) =>
      TurnLifecycle.admit(tx, {
        ...
        envelope: { fixture: "frozen_gate19" },
        policyBasis: { fixture: "frozen_gate19" },
        timeAdmitted: 100,
      }),
    )
    yield* db.run(sql`
    ...
    yield* db.transaction((tx) =>
      admitLegacyModelWithoutLearningContext(tx, {
        ...
        sessionID,
        assistantMessageID,
        requestEnvelope: { prompt: `frozen learning context v${generation} request` },
...
function dropGate20A(db: TestDatabase) {
  return Effect.gen(function* () {
    ...
    const triggers = yield* db.all<{ name: string }>(sql`
      ...
      WHERE type = 'trigger' AND name LIKE 'assignment_%'
    `)
    yield* Effect.forEach(triggers, (trigger) => db.run(sql`DROP TRIGGER ${sql.identifier(trigger.name)}`), {
      discard: true,
    })
    yield* Effect.forEach(assignmentTables, (table) => db.run(sql`DROP TABLE IF EXISTS ${sql.identifier(table)}`), {
      discard: true,
    })
...
function dropGate21(db: TestDatabase) {
  return Effect.gen(function* () {
    ...
    const triggers = yield* db.all<{ name: string }>(sql`
      SELECT name FROM sqlite_master
      WHERE type = 'trigger' AND name LIKE 'advisory_plan_suggestion_%'
    `)
    yield* Effect.forEach(triggers, (trigger) => db.run(sql`DROP TRIGGER ${sql.identifier(trigger.name)}`), {
      discard: true,
    })
    yield* Effect.forEach(
      advisoryPlanSuggestionTables,
      (table) => db.run(sql`DROP TABLE IF EXISTS ${sql.identifier(table)}`),
      { discard: true },
    )
    yield* db.transaction((tx) => installSchemaExtrasV22(tx))
    yield* db.run(sql`DELETE FROM repa_migration WHERE version = ${BASELINE_VERSION + 22}`)
    yield* db.run(sql.raw("PRAGMA foreign_keys = ON"))
  })
}

function dropGate20(db: TestDatabase) {
  return Effect.gen(function* () {
    yield* dropGate20A(db)
    yield* db.run(sql.raw("PRAGMA foreign_keys = OFF"))
    const triggers = yield* db.all<{ name: string }>(sql`
      SELECT name FROM sqlite_master
      WHERE type = 'trigger'
        AND (name LIKE 'future_attention_%'
          OR name LIKE 'turn_candidate_future_attention_service_source_%'
          OR name LIKE 'turn_unavailable_model_terminal_%'
          OR sql LIKE '%future_attention_%')
    `)
    yield* Effect.forEach(triggers, (trigger) => db.run(sql`DROP TRIGGER ${sql.identifier(trigger.name)}`), {
      discard: true,
    })
    ...
    yield* db.run(sql`ALTER TABLE turn_tool_candidate DROP COLUMN future_attention_service_source`)
    yield* db.run(sql`ALTER TABLE turn_unavailable_model DROP COLUMN time_settled`)
    yield* db.run(sql`ALTER TABLE turn_unavailable_model DROP COLUMN state`)
    yield* db.run(sql`DELETE FROM repa_migration WHERE version = ${BASELINE_VERSION + 19}`)
    yield* db.run(sql.raw("PRAGMA foreign_keys = ON"))
  })
}

function dropGate19(db: TestDatabase) {
  return Effect.gen(function* () {
    yield* dropGate20(db)
    yield* db.run(sql.raw("PRAGMA foreign_keys = OFF"))
    const triggers = yield* db.all<{ name: string }>(sql`
      SELECT name FROM sqlite_master
      WHERE type = 'trigger'
        AND (name LIKE 'learner_response_evidence_%' OR sql LIKE '%learner_response_evidence_%')
    `)
    yield* Effect.forEach(triggers, (trigger) => db.run(sql`DROP TRIGGER ${sql.identifier(trigger.name)}`), {
      discard: true,
    })
    ...
    yield* db.run(sql`DELETE FROM repa_migration WHERE version = ${BASELINE_VERSION + 18}`)
    yield* db.run(sql.raw("PRAGMA foreign_keys = ON"))
  })
}
...
describe("DatabaseMigration", () => {
  ...
  test("upgrades a frozen Gate 16 database through Gate 20 without fabricating bootstrap or context state", async () => {
    await using tmp = await tmpdir()
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        ...
        return {
          ...
          `),
          ownerMap: yield* Effect.flatMap(MaterialMap.Service, (maps) =>
            maps.getMap("map_gate17_fixture" as MaterialMap.MapID),
          ).pipe(Effect.provide(materialMapLayer({ db, filename }))),
        }
      }),
      filename,
    )
    const ownerMap = upgraded.ownerMap
    ...
    expect(upgraded.ownerMap.target).toMatchObject({
      type: "artifact",
      authorization: {
  ...
  test("upgrades a frozen Gate 17 database through Gate 20 while preserving the Gate 18 schema", async () => {
    const fresh = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* DatabaseMigration.apply(db)
  ...
  test("upgrades a frozen Gate 19 database through current migrations without rewriting its v2 cut", async () => {
    const fresh = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* DatabaseMigration.apply(db)
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* initializeDatabaseV19(db)
        expect(yield* db.get<Record<string, number>>(sql.raw("PRAGMA user_version"))).toEqual({
          user_version: BASELINE_VERSION + databaseV19Migrations.length,
        })
        expect(
          yield* db.get(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND name = 'future_attention_concern'
          `),
        ).toBeUndefined()

        const seeded = yield* seedFrozenLearningContextCut(db)
        yield* DatabaseMigration.apply(db, { path: "frozen-gate19.db" })
        ...
        return {
          ...
          tables: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND name LIKE 'future_attention_%'
            ORDER BY name
          `),
          triggers: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'trigger' AND name LIKE 'future_attention_%'
            ORDER BY name
          `),
          rows: yield* db.get(sql`
            SELECT
              (SELECT count(*) FROM future_attention_change_set) AS changeSets,
              (SELECT count(*) FROM future_attention_concern) AS concerns,
              (SELECT count(*) FROM future_attention_transition) AS transitions,
              (SELECT count(*) FROM future_attention_claim_group) AS claimGroups,
              (SELECT count(*) FROM future_attention_claim_finalization) AS finalizations
          `),
          foreignKeys: yield* db.all(sql.raw("PRAGMA foreign_key_check")),
        }
      }),
    ...
    expect(upgraded.before).toEqual(upgraded.after)
    expect(upgraded.after.type).toBe("available")
    if (upgraded.after.type !== "available") throw new Error("Expected preserved Gate 19 cut")
    expect(upgraded.after.cut.policyVersion).toBe(2)
    expect(upgraded.after.cut.rendererVersion).toBe(2)
    ...
    expect(upgraded.triggers).toHaveLength(15)
    expect(upgraded.rows).toEqual({ changeSets: 0, concerns: 0, transitions: 0, claimGroups: 0, finalizations: 0 })
    expect(upgraded.foreignKeys).toEqual([])
  })

  test("upgrades a frozen Gate 20 database through Gate 20A without rewriting its v3 cut or fabricating Assignments", async () => {
    const fresh = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* initializeDatabaseV20(db)
        expect(yield* db.get<Record<string, number>>(sql.raw("PRAGMA user_version"))).toEqual({
          user_version: BASELINE_VERSION + databaseV20Migrations.length,
        })
        expect(
          yield* db.get(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND name = 'assignment'
          `),
        ).toBeUndefined()

        const beforeJournal = yield* db.all(sql`SELECT version, id FROM repa_migration ORDER BY version`)
        const seeded = yield* seedFrozenLearningContextCut(db, 3)
        ...
        return {
          ...
          tables: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND (name = 'assignment' OR name LIKE 'assignment_%')
            ORDER BY name
          `),
          triggers: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
          ...
          rows: yield* db.get(sql`
            SELECT
              ...
              (SELECT count(*) FROM assignment_no_change_seal) AS noChangeSeals,
              (SELECT count(*) FROM assignment_commit_seal) AS seals
          `),
          revisionValidationTrigger: yield* db.get<{ definition: string }>(sql`
            SELECT sql AS definition
            FROM sqlite_schema
            WHERE type = 'trigger' AND name = 'assignment_revision_insert_validate'
          `),
    ...
    expect(upgraded.assignmentBehavior).toEqual(fresh.assignmentBehavior)
    expect(upgraded.assignmentBehavior).toMatchObject({
      ownerCut: { frontierSequence: 0, frontierTime: 0, headCount: 0 },
      asOf: 200,
      countAtCut: 0,
    ...
    expect(upgraded.beforeJournal).toEqual([
      { version: BASELINE_VERSION, id: BASELINE_ID },
      ...databaseV20Migrations.map((migration, index) => ({
    ...
    expect(upgraded.journal.at(-1)).toEqual({
      version: BASELINE_VERSION + databaseV20Migrations.length + 1,
      id: assignmentAuthorityMigration.id,
    })
    expect(upgraded.beforeStored).toEqual(upgraded.afterStored)
    expect(upgraded.before).toEqual(upgraded.after)
    expect(upgraded.after.type).toBe("available")
    if (upgraded.after.type !== "available") throw new Error("Expected preserved Gate 20 cut")
    expect(upgraded.after.cut.policyVersion).toBe(3)
    expect(upgraded.after.cut.rendererVersion).toBe(3)
    ...
    expect(upgraded.after.cut.sections.some((section) => section.owner === "assignment")).toBe(false)
    expect(upgraded.tables.map((row) => row.name)).toEqual([
      "assignment",
      "assignment_capability_issue",
      "assignment_capability_settlement",
    ...
    expect(upgraded.triggers).toHaveLength(26)
    expect(upgraded.rows).toEqual({ assignments: 0, revisions: 0, effects: 0, noChangeSeals: 0, seals: 0 })
    expect(upgraded.revisionValidationTrigger?.definition).toContain(
    ...
    expect(upgraded.userVersion).toEqual({
      user_version: BASELINE_VERSION + databaseV20Migrations.length + 1,
    })
  ...
  test("upgrades a frozen Gate 20A database through Gate 20B without rewriting its v4 cut or fabricating learner state", async () => {
    const fresh = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* initializeDatabaseV22(db)
        return {
          manifest: yield* completeSchemaManifest(db),
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        ...
        const beforeStored = yield* db.get(sql`
          ...
          WHERE assistant_message_id = ${seeded.assistantMessageID}
        `)
        yield* DatabaseMigration.apply(db, { path: "frozen-gate20a.db", migrations: databaseV22Migrations })
  ...
  test("upgrades a frozen Gate 20B database through Gate 21 without rewriting its v5 cut or fabricating advice", async () => {
    const fresh = await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* DatabaseMigration.apply(db)
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        ...
        yield* initializeDatabaseV22(db)
        expect(yield* db.get<Record<string, number>>(sql.raw("PRAGMA user_version"))).toEqual({
          user_version: BASELINE_VERSION + databaseV22Migrations.length,
        })
        expect(
          yield* db.get(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND name = 'advisory_plan_suggestion'
          `),
        ).toBeUndefined()

        const beforeJournal = yield* db.all(sql`SELECT version, id FROM repa_migration ORDER BY version`)
        const seeded = yield* seedFrozenLearningContextCut(db, 5)
        const beforeStored = yield* db.get(sql`
          SELECT
        ...
        return {
          ...
          tables: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'table' AND name LIKE 'advisory_plan_suggestion%'
            ORDER BY name
          `),
          tableRows: yield* Effect.forEach(advisoryPlanSuggestionTables, (table) =>
            db
              .get<{ count: number }>(sql`SELECT count(*) AS count FROM ${sql.identifier(table)}`)
              .pipe(Effect.map((row) => ({ table, count: row?.count ?? -1 }))),
          ),
          triggers: yield* db.all<{ name: string }>(sql`
            SELECT name FROM sqlite_schema
            WHERE type = 'trigger'
              AND (name LIKE 'advisory_plan_suggestion_%'
                OR name = 'learning_command_invocation_terminal_validate_v23')
            ORDER BY name
          `),
          userVersion: yield* db.get<Record<string, number>>(sql.raw("PRAGMA user_version")),
          foreignKeysEnabled: yield* db.get<Record<string, number>>(sql.raw("PRAGMA foreign_keys")),
    ...
    expect(upgraded.beforeJournal).toEqual([
      { version: BASELINE_VERSION, id: BASELINE_ID },
      ...databaseV22Migrations.map((migration, index) => ({
        version: BASELINE_VERSION + index + 1,
        id: migration.id,
      })),
    ])
    expect(upgraded.journal.slice(0, -1)).toEqual(upgraded.beforeJournal)
    expect(upgraded.journal.at(-1)).toEqual({
      version: BASELINE_VERSION + databaseV22Migrations.length + 1,
      id: advisoryLearningPlanningMigration.id,
    })
    expect(upgraded.beforeStored).toEqual(upgraded.afterStored)
    expect(upgraded.before).toEqual(upgraded.after)
    expect(upgraded.after.type).toBe("available")
    if (upgraded.after.type !== "available") throw new Error("Expected preserved Gate 20B cut")
    expect(upgraded.after.cut.policyVersion).toBe(5)
    expect(upgraded.after.cut.rendererVersion).toBe(5)
    ...
    expect(upgraded.after.cut.sections.some((section) => section.owner === "advisory_plan_suggestion")).toBe(false)
    expect(upgraded.tables.map((row) => row.name)).toEqual([...advisoryPlanSuggestionTables].toSorted())
    expect(upgraded.tableRows.every((row) => row.count === 0)).toBeTrue()
  ...
  test("upgrades the frozen v12 schema through v13 to exact current Default-Course structural parity", async () => {
    ...
    const upgraded = await run(
      Effect.gen(function* () {
        ...
        yield* db.transaction((tx) => advisoryLearningPlanningMigration.up(tx))
        yield* db.run("PRAGMA foreign_keys = ON")
        return {
