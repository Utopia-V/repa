...
describe("Config", () => {
  ...
  it.live("loads Repa JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "repa.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "repa.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "repa.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "repa.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".repa"), { recursive: true })
            await fs.mkdir(path.join(directory, ".repa"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "repa.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "repa.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "repa.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "repa.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "repa.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".repa", "repa.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".repa", "repa.jsonc"),
