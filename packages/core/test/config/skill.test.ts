    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect(
        ...
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.repa") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.repa", "skill")),
        }),
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.repa", "skills")),
        }),
