...
/** Frozen Gate 20B producer copied at the accepted v5 boundary. */
export function prepareFrozenGate20BLearningContextCut(input: {
  ...
  const providerToolSurface = bindProviderToolSurface({
    ...
    definitions: [
      {
        id: "advisory_plan_suggestion_read",
        value: {
          type: "function",
          name: "advisory_plan_suggestion_read",
          description: "frozen Gate 20B custom tool",
        },
      },
    ],
  })
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (nextCanonicalBytes === canonicalBytes && cut.budget.renderedBytes === renderedBytes) {
      if (nextCanonicalBytes > hardLimits.canonicalBytes || renderedBytes > hardLimits.renderedBytes) {
        throw new Error("Frozen Gate 20B learning context exceeds its historical limits")
      }
      return { cut, canonicalCut, renderedBlock }
    }
    canonicalBytes = nextCanonicalBytes
  }
  throw new Error("Frozen Gate 20B learning-context byte accounting did not converge")
}
...
function toJsonValue(value: unknown): JsonValue {
  ...
  if (encoded === undefined) throw new Error("Frozen Gate 20B value is not representable as JSON")
