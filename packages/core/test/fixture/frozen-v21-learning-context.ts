...
/**
 * Frozen Gate 20A producer copied at the accepted v4 boundary. It owns its
 * historical catalog, section algebra, renderer, canonicalization, and limits;
 * current LearningContext code is deliberately absent from this fixture.
 */
export function prepareFrozenGate20ALearningContextCut(input: {
  ...
  const providerToolSurface = bindProviderToolSurface({
    ...
    definitions: [
      {
        id: "learner_state_judgment_read",
        value: {
          type: "function",
          name: "learner_state_judgment_read",
          description: "frozen Gate 20A custom tool",
        },
      },
    ],
  })
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (nextCanonicalBytes === canonicalBytes && cut.budget.renderedBytes === renderedBytes) {
      if (nextCanonicalBytes > hardLimits.canonicalBytes || renderedBytes > hardLimits.renderedBytes) {
        throw new Error("Frozen Gate 20A learning context exceeds its historical limits")
      }
      return { cut, canonicalCut, renderedBlock }
    }
    canonicalBytes = nextCanonicalBytes
  }
  throw new Error("Frozen Gate 20A learning-context byte accounting did not converge")
}
...
function toJsonValue(value: unknown): JsonValue {
  ...
  if (encoded === undefined) throw new Error("Frozen Gate 20A value is not representable as JSON")
