...
describe("transcript", () => {
  describe("formatAssistantHeader", () => {
    ...
    test("includes metadata when enabled", () => {
      ...
      expect(result).toBe("## Assistant (Repa · claude-sonnet-4-20250514 · 5.4s)\n\n")
    })

    test("uses model display name when available", () => {
      const result = formatAssistantHeader(baseMsg, true, providers)
      expect(result).toBe("## Assistant (Repa · Claude Sonnet 4 · 5.4s)\n\n")
    })
    ...
    test("handles missing completed time", () => {
      ...
      expect(result).toBe("## Assistant (Repa · claude-sonnet-4-20250514)\n\n")
    })
  ...
  describe("formatMessage", () => {
    ...
    test("formats assistant message with metadata", () => {
      ...
      const result = formatMessage(msg, parts, options)
      expect(result).toContain("## Assistant (Repa · Claude Sonnet 4 · 5.4s)")
  ...
  describe("formatTranscript", () => {
    test("formats complete transcript", () => {
      ...
      expect(result).toContain("## User")
      expect(result).toContain("Hello")
      expect(result).toContain("## Assistant (Repa · Claude Sonnet 4 · 0.5s)")
    ...
    test("falls back to raw model id when provider data is missing", () => {
      ...
      expect(result).toContain("## Assistant (Repa · claude-sonnet-4-20250514 · 0.5s)")
    })
    ...
    test("formats transcript without assistant metadata", () => {
      ...
      expect(result).not.toContain("Repa")
