...
export const REPA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const REPA_MODE_KEY = "repa.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(REPA_MODE_KEY, REPA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(REPA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(REPA_MODE_KEY, stack.at(-1)?.mode ?? REPA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? REPA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(REPA_MODE_KEY, undefined)
