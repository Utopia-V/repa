...
const NO_MODELS_TIP = "Run {highlight}/connect{/highlight} to add an AI provider and start learning or working"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("repa.status"),
...
const TIPS: Tip[] = [
  ...
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between Repa and Plan profiles"),
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.repa/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.repa/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.repa/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.repa/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Repa from reading sensitive files",
  "Use {highlight}repa run{/highlight} for non-interactive scripting",
  "Use {highlight}repa --continue{/highlight} to resume the last session",
  "Use {highlight}repa run -f file.ts{/highlight} to attach files via CLI",
  ...
  "Run {highlight}repa debug config{/highlight} to troubleshoot configuration",
