  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "repa")
        draft.active = typeof active === "string" ? active : "repa"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "repa")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.repa, store.mode)
    })
