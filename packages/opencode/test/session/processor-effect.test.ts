...
it.live("session.processor reconciles a committed Default-Course proposal when terminal notification interrupts", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        yield* llm.tool(PROPOSE_DEFAULT_COURSE_PREFERENCE_TOOL_ID, input)

        const { chat, parent } = yield* runningSession("proposal commit before observer interruption")
        const msg = yield* assistant(chat.id, parent.id, path.resolve(dir))
        const mdl = yield* provider.getModel(ref.providerID, ref.modelID)
        const handle = yield* processors.create({ assistantMessage: msg, sessionID: chat.id, model: mdl })
        const frontierBefore = yield* database.db.transaction((tx) => LearningFrontier.read(tx))
        let registration: SessionProcessor.RegisteredToolCall | undefined
        let bodyRuns = 0
        const proposal = Object.assign(
          tool({
            description: "Record one host-prepared Default-Course proposal",
            inputSchema: z.object({
              expectedHeadID: z.string().nullable(),
              expectedVersion: z.number(),
              target: z.null(),
              resolutionScope: z.object({
                coverage: z.literal("complete"),
                candidates: z.array(z.never()),
                selectedCourseID: z.null(),
              }),
            }),
            execute: async () => {
              bodyRuns++
              return { title: "Late proposal body", output: "late", metadata: { late: true } }
            },
          }),
          {
            [SessionProcessor.ToolCallPreparation]: async (
              attempt: unknown,
              prepared: SessionProcessor.RegisteredToolCall,
            ) => {
              registration = prepared
              await bridge.promise(runtime.prepareDefaultCourseProposal(attempt, prepared))
            },
          },
        )
        let observerRuns = 0
        const unsubscribe = yield* events.listen((event) => {
          ...
          if (
            ...
            data.part.type !== "tool" ||
            data.part.tool !== PROPOSE_DEFAULT_COURSE_PREFERENCE_TOOL_ID ||
        ...
        const outcome = yield* handle.process({
          ...
          agent: agent(),
          system: [],
          messages: [{ role: "user", content: "record the bounded proposal" }],
          tools: { [PROPOSE_DEFAULT_COURSE_PREFERENCE_TOOL_ID]: proposal },
        })
        const call = (yield* MessageV2.parts(msg.id)).find(
          (part): part is SessionV1.ToolPart =>
            part.type === "tool" && part.tool === PROPOSE_DEFAULT_COURSE_PREFERENCE_TOOL_ID,
        )
        const invocation = registration
          ? yield* database.db.transaction((tx) =>
              TurnLifecycle.invocation(tx, { turnID: registration!.turnID, partID: registration!.partID }),
        ...
        expect(call?.state.status).toBe("completed")
        if (!call || call.state.status !== "completed") return
        expect(JSON.parse(call.state.output)).toMatchObject({ outcome: "proposal_recorded" })
        expect(call.state.metadata).toMatchObject({ durablyRecorded: true, mutating: false })
        expect(msg.error).toBeUndefined()
        expect(invocation?.state).toBe("completed")
        expect(yield* database.db.transaction((tx) => LearningFrontier.read(tx))).toEqual(frontierBefore)
        expect(
          yield* database.db.get(sql`
            SELECT
              (SELECT count(*) FROM ${LearnerDefaultCourseProposalTable}
                WHERE part_id = ${call.id}) AS proposals,
              (SELECT count(*) FROM ${LearningCommandInvocationTable}
                WHERE part_id = ${call.id}) AS physical_invocations,
              (SELECT count(*) FROM ${LearnerDefaultCourseDispositionTable}
                WHERE invocation_part_id = ${call.id}) AS dispositions,
              (SELECT count(*) FROM ${LearnerDefaultCourseCapabilityIssueTable}
                WHERE invocation_part_id = ${call.id}) AS capability_issues,
              (SELECT count(*) FROM ${LearnerDefaultCourseCapabilitySettlementTable}
                WHERE invocation_part_id = ${call.id}) AS capability_settlements,
              (SELECT count(*) FROM ${DefaultCoursePreferenceTransitionTable}
                WHERE authorization_part_id = ${call.id}) AS effects
          `),
        ).toEqual({
          proposals: 1,
