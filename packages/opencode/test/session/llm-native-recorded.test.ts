...
function decodeRecordOpenAIOAuth() {
  const value = process.env.REPA_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("REPA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("REPA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    canRecord: () => Boolean(process.env.REPA_RECORD_CONSOLE_TOKEN && process.env.REPA_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        ...
        api: zenURL(process.env.REPA_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.REPA_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.REPA_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        model,
        options: {
          apiKey: envValue("REPA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("REPA_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
