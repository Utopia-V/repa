 ...
 *
 * The script intentionally isolates `REPA_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "repa.jsonc"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: ctx.headers(),
      body: { agent: "repa", model: { providerID: "test", modelID: "test-model" }, command: "printf shell-ok" },
    }))
    .json(
      200,
      (body) => {
