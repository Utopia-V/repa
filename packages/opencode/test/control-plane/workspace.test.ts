...
const originalEnv = {
  ...
  REPA_EXPERIMENTAL_WORKSPACES: process.env.REPA_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.REPA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.REPA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.REPA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.REPA_EXPERIMENTAL_WORKSPACES).toBe("true")
