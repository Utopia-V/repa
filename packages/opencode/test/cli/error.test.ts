...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/repa.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/repa.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/repa.jsonc", dir: ".reap", suggestion: ".repa" },
        expected:
          'Directory ".reap" in /tmp/repa.jsonc is not valid. Rename the directory to ".repa" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/repa.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `repa models` to list available models",
      "Or check your config (repa.json) provider/model names",
    ].join("\n")
