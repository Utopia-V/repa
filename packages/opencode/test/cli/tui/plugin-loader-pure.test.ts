...
`,
  ...
  const pure = process.env.REPA_PURE
  const meta = process.env.REPA_PLUGIN_META_FILE
  process.env.REPA_PURE = "1"
  process.env.REPA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.REPA_PURE
    } else {
      process.env.REPA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.REPA_PLUGIN_META_FILE
    } else {
      process.env.REPA_PLUGIN_META_FILE = meta
    }
  }
})
