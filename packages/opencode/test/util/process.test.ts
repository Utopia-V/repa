...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.REPA_TEST ?? "")'), {
      env: {
        REPA_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "REPA_TEST_SHELL"], {
      shell: true,
      env: {
        REPA_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("REPA_TEST_SHELL=ok")
  })
