...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "repa.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "repa.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "repa.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "repa.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["repa.json", "repa.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "repa.json"),
        path.join(project, "repa.jsonc"),
      ])
    })
  })
