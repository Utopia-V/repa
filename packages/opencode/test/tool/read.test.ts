...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.REPA_REPO_CLONE_GITHUB_BASE_URL
      process.env.REPA_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.REPA_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.REPA_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.read env file permissions", () => {
  ...
  for (const agentName of ["repa", "plan"] as const) {
          ...
          Effect.gen(function* () {
            ...
            const asked = yield* provideInstance(dir)(
              Effect.gen(function* () {
                ...
                if (!info) throw new Error(`missing test agent: ${agentName}`)
                let asked = false
                const next = {
                  ...ctx,
                  ask: (req: Omit<PermissionV1.Request, "id" | "sessionID" | "tool">) =>
