  ...
  return {
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Title": InstallationProductName,
            "X-Source": InstallationProductID,
          },
        },
      }),
    openrouter: () =>
    ...
    nvidia: (provider) =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Title": InstallationProductName,
            "X-BILLING-INVOKE-ORIGIN": InstallationProductName,
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `${InstallationUserAgent} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `${InstallationUserAgent} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `repa auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `${InstallationUserAgent} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": InstallationProductID,
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, repa auth, or provider options.`,
            )
          },
        }
      }
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [id, fn] of Object.entries(custom(dep))) {
          ...
          // Explicit configuration activates the provider even when its custom loader is not environment-autoloaded.
          if (result && (result.autoload || providers[providerID] || data.source === "config")) {
