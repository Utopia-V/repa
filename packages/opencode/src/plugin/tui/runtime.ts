...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".repa"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".repa", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".repa", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.REPA_PURE ? [] : pluginOrigins
    if (Flag.REPA_PURE && pluginOrigins.length) {
    }
