  ...
  return {
    ...
    auth: {
      ...
      async loader(getAuth) {
        ...
        const rewrite = (requestInput: RequestInfo | URL, init?: RequestInit) => {
          const parsed =
        ...
        const oauthFetch = async (requestInput: RequestInfo | URL, init?: RequestInit) => {
          ...
          const routed = rewrite(requestInput, init)
          const requestInit = {
            ...routed.init,
            headers,
          }
          if (
            websocketFetch &&
            new URL(routed.input instanceof Request ? routed.input.url : routed.input).pathname.endsWith("/responses")
          ) {
            return websocketFetch(routed.input, requestInit)
          }
          return fetch(routed.input, OpenAIWebSocketPool.withoutInternalHeaders(requestInit))
        }
        ...
        return {
          ...
          fetch: websocketFetch
            ? oauthFetch
            : ProviderWire.routeFetch(oauthFetch as typeof fetch, {
                identity: "openai-codex-oauth-http-v1",
                rewrite,
              }),
        }
      },
