...
`../llm.ts` is the Repa session LLM service. It owns Repa concerns: auth, config, model/provider resolution, plugins, permissions, telemetry headers, and runtime selection. It is the only file in this area that should know about the full session request shape.
...
- `native-request.ts` converts Repa's normalized session input into a native `@opencode-ai/llm` `LLMRequest`. It does not execute requests.
- `native-runtime.ts` is the opt-in native runtime adapter. It decides whether a selected model is supported, builds the native request, bridges Repa tools into native executable tools, and delegates transport to `LLMClient` / `RequestExecutor`.
...
src/session/
  ...
  llm/
    ...
    native-request.ts       Repa/AI SDK-shaped input -> @opencode-ai/llm LLMRequest
...
`native-runtime.ts` evaluates the gate and either bridges into `@opencode-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays Repa-owned in both branches; only request lowering and transport differ.
