...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text(["repa", "import", sessionUrl], { nothrow: true }),
          )
    ...
    const repaArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["repa", ...repaArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`repa exited with code ${code}`))
  }),
})
