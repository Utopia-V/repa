...
try {
    ...
    InstanceStore.Service.use((store) =>
      store.load({ directory: process.cwd() }).pipe(
        Effect.flatMap((ctx) =>
          ...
          const session = yield* sessions.create({ title: "Gate 11 real configured-model evidence" })
          ...
          const input = {
            ...
            producer: { kind: "configured_model" as const, sessionID: session.id, messageID },
          ...
          if (!decoded.ok) return yield* Effect.die(`The accepted configured-model profile is invalid: ${decoded.error}`)
  ...
  await import("../src/server/server")
    .then(({ Server }) => Server.disposeDefault())
    .catch(() => {})
